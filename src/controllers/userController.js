...
const { isAdmin, isSameUser, returnRole } = require('../middlewares/authorizationMiddleware');
...
const updateUser = async (req, res) => {
    try {
        ...
        const authorized = await isSameUser(tokenHeader);
        if (authorized === "expired") {
            return res.status(403).json({ "message": "Token expired!" });
        } else if (!authorized) {
            return res.status(403).json({ "message": "Not authorized!" });
        ...
        const userToUpdate = await User.findByPk(token.id);
        if (!userToUpdate) {
            return res.status(404).json({ "message": "User not found" });
        }
        ...
        const emailExists = await User.findOne({ where: { email: email } });
        if (emailExists && emailExists.user_id !== token.id) {
            return res.status(400).json({ "message": "User with this email already exists!" });
        }
        ...
        let updatedUser;
         ...
         updatedUser = await User.update({ name: name, email: email }, { where: { user_id: token.id} });


        if (updatedUser == 1) {
            const user = await User.findByPk(token.id, { attributes: ['user_id', 'name', 'email', 'role'] });
            return res.status(200).json(user);
        } else {
            return res.status(500).json({ "message": "Error when updating user!" });
        }
...
const user = async (req, res) => {
    ...
    try {
        ...
        const authorized = await returnRole(tokenHeader);
